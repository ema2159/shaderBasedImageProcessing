...
vec3 nn_iterp(vec2 frag_pos, sampler2D image, vec2 cellSize) {
  ...
  float h_coord = (pix_mod.x <= 0.5) ? frag_pos.x - pix_mod.x
                                     : frag_pos.x - pix_mod.x + cellSize.x;
  float v_coord = (pix_mod.y <= 0.5) ? frag_pos.y - pix_mod.y
                                     : frag_pos.y - pix_mod.y + cellSize.y;
